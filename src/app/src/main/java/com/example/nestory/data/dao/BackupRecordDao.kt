...
interface BackupRecordDao {
    ...
    fun observeAllBackupRecords(): Flow<List<BackupRecordEntity>>

    @Query("SELECT * FROM backup_records ORDER BY created_at DESC")
    suspend fun getAllBackupRecords(): List<BackupRecordEntity>

    @Query("SELECT * FROM backup_records ORDER BY created_at DESC LIMIT 1")
    fun observeLatestBackup(): Flow<BackupRecordEntity?>

    @Query("SELECT * FROM backup_records ORDER BY created_at DESC LIMIT 1")
    suspend fun getLatestBackup(): BackupRecordEntity?
